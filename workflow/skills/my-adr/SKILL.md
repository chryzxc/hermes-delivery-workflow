...
Lightweight Architecture Decision Records: the durable trail of *why* the code is shaped the way it is. Planner authors and amends ADRs during planning; Reviewer verifies changed behavior against them; nobody re-litigates a decision that has an accepted ADR without superseding it.
...
3. Write the ADR before implementation merges (Planner includes it in the plan's work packets).
4. Status lifecycle: `proposed` → `accepted` (the operator or delegated owner) → `superseded by NNNN`. Never `accepted` without a named decision maker.
