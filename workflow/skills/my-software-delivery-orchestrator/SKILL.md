...
- Before any multi-card dispatch, read the live engine caps (`kanban.max_in_progress` and `kanban.max_in_progress_per_profile` in `~/.hermes/config.yaml`) and state the actual concurrency and wave plan in the dispatch report (for example: "5 cards, per-profile cap 3 → 2 implement waves plus pipelined reviews"). Never promise more parallelism than the engine caps allow. When creating more than 3 cards in one wave, the creation report itself states that wave plan and marks which cards are LOW fast-lane, so the operator sees expected completion order up front.
