...
Nexus is the sole software-delivery control plane. Hermes Bot profiles are the durable specialists; this skill routes work from product clarification through implementation, QA, security, release evidence, and PR preparation. It preserves authority and evidence without duplicating specialist mandates, identities, or procedures. General IT operations and autonomous deployment are outside this skill.

## Ownership model

| Responsibility | Owner |
| --- | --- |
| User intent, authorization, task graph, integration, final report | Nexus |
| Final PR title, body, metadata, and authorized publication | Nexus using `creating-pr-content` |
| External research and source verification | Atlas |
| Small deterministic repository mapping | Nexus direct tools |
| Cross-cutting repository discovery, architecture, ADRs, and implementation plans | Archon |
| Product, UI/UX, and accessibility design | Canvas |
| Backend, frontend, and full-stack implementation | Forge |
| DevOps, CI/CD, and operational readiness | Aegis |
| Regression, CI, and release evidence | Sentinel |
| Independent quality and code review | Sentry |
| Security analysis and security remediation advice | Cypher |
| Authorized active security validation and engagement evidence | Wraith |

Christian is final authority. A specialist may not assume another specialist's mandate.

## Security and delivery boundaries

- A codebase security audit routes to Cypher as a read-only audit.
- Active testing never starts from a suggestion alone. Cypher may request a blocked Wraith engagement; Christian must approve its engagement contract before dispatch.
- Wraith validates; Cypher interprets security significance; Forge remediates; Sentry reviews; Sentinel runs triggered regression evidence.
- Aegis is used here only for CI/CD, build, container, IaC, rollback, and release preparation. General IT operations are outside this skill.
- This orchestrator prepares release evidence but never authorizes or performs deployment.

## Authority and approval gates

Use the lowest authority implied by the request. Local, scoped, reversible work may proceed when authorized. Christian's explicit approval is required before external communication or publishing; merge, deployment, production changes or migrations; purchases or billing; credential changes; and irreversible deletion.
...
The orchestrator skill stays on **Nexus only**. Do not copy it into specialist profiles: Nexus owns routing and reconciliation; each Bot owns its role-local SOUL charter and procedure skill.
...
3. Use direct Nexus tools for deterministic discovery, one-command checks, and tiny single-owner work.
...
5. Use **manual Kanban** for every material task that needs Bot identity, profile-specific model/skills/memory, local edits, review, recovery, durable evidence, dependencies, or human interruption. The current installation has `kanban.auto_decompose: false`; Nexus explicitly creates and assigns the cards.
...
11. Use native same-card review for an ordinary implementation slice: Forge requests Sentry review; Sentry completes, requests changes, or blocks that card. Create separate linked QA/security/operations cards only when their independent gate is actually distinct. Dispatch only the current executable slice.
12. After final gates and explicit PR authorization, Nexus loads `creating-pr-content`, generates the title/body/metadata from verified evidence, and invalidates that content if the head changes. Specialists supply evidence; they do not produce competing final PR descriptions.
...
- Treat worker summaries as hypotheses until Nexus inspects current evidence.
- Freeze changed state before Sentinel, Sentry, or Cypher reviews it. Reviewers do not edit; accepted findings go to the owning implementer in a new bounded task.
...
- Meaningful code diffs require the distinct OCR gate and independent Sentry review under the applicable Bot skills and task brief. Missing gates are blockers, never passes.
- For any changed behavior that crosses a component, process, service, persistence, or external boundary, the implementation handoff and Sentry review brief must include an explicit end-to-end contract trace (initiating input/state → each handoff's type/identity/shape → receiving lookup/mutation → returned state consumed next) and a named real-path regression test. Sentry must issue `REQUEST_CHANGES` when either is absent; presence, ordering, mocked-unit, isolated-helper, or visual-preview tests are not substitutes.
- Route security-triggered work to Cypher and research to Atlas. Nexus may synthesize their findings but does not substitute for their analysis.
...
Run coordination sessions short: one material task per session, final report, then end it. Heavy tool output (diffs, file reads, test logs) belongs on Kanban cards and worker sessions, never in a growing coordinator transcript — a marathon Nexus session re-pays its whole context every turn. Independent gates (Sentry, Sentinel, Cypher) review the same frozen SHA concurrently as separate cards; Nexus applies the most severe verdict and holds the card until all return. Batch queued small cards in the same module into one Forge session (max 3, zero shared files, one frozen commit per card) to amortize session startup. Route mechanical diffs (≤2 files, no contract change, existing coverage) through the fast lane: Forge direct plus same-card Sentry review. Any check expressible as a script runs without an LLM; LLM passes only triage pre-filtered script output or review diffs. Every card carries a `token_budget`; workers return `BLOCKED: budget exceeded` instead of grinding. Full policy: `references/parallel-execution.md` and `team-config.yaml` (`parallelism`, `token_policy`).

## Completion

Nexus reports the assigned Bot(s), actual execution backend and observed placement, scope, evidence, findings, residual risk, approval boundaries, and any blocked work. `DONE` requires current evidence for every acceptance criterion and every required final-state gate. `HARD_BLOCKED` is incomplete work, never done.
