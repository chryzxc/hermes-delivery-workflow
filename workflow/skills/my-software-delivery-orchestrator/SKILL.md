...
- On every terminal notification/wake, inspect the task event, latest run outcome and metadata, summary, comments, and dependencies before reporting or advancing. A generic `done` status never overrides a recorded review verdict or missing successor. A dispatch turn does not end while any card in the wave lacks a verified `notify+wake` subscription (`hermes kanban notify-list <id> --json` proof on the card). Progress reaches the operator through push — terminal wakes via subscriptions and the supervisor's change digest — never through the operator asking, and never through foreground watching (`sleep`, polling, process-manager checks).
- Every material terminal reconciliation records exactly ONE comment on the card starting with the literal tag `CONTINUATION:` followed by exactly one decision — `same-card correction`, `promote gate <card-id>`, `successor <card-id>`, `blocker <name> (owner <profile>)`, or `final report` — plus its evidence (ids/URLs). Replayed terminal notifications are no-ops when the marker already exists after the terminal event. A `done` card without a post-completion `CONTINUATION:` marker is an orphaned delivery chain; before any manual promote or dispatch, verify canonical status, workspace validity, and that no newer card supersedes the same scope (record supersession as a comment linking the replacement card-id).
