...
The final go-live gate, run by Sentinel (evidence owner) with Aegis (operational owner) supplying the ops sections. Verdict: `GO`, `GO_WITH_RISKS` (named, accepted by Christian), or `NO_GO` — recorded on the release card with evidence per line. Nothing reaches production without this pass.
...
**Security (Cypher verdict referenced, not duplicated)**
...
- `GO_WITH_RISKS` lists every accepted risk with Christian's named acceptance.
